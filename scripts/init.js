...
if (!fs.existsSync(path.join(braveCoreDir, '.git'))) {
  Log.status(`Cloning presearch-core [${braveCoreRef}] into ${braveCoreDir}...`)
  fs.mkdirSync(braveCoreDir)
  util.runGit(braveCoreDir, ['clone', util.getNPMConfig(['projects', 'presearch-core', 'repository', 'url']), '.'])
  util.runGit(braveCoreDir, ['checkout', braveCoreRef])
}
const braveCoreSha = util.runGit(braveCoreDir, ['rev-parse', 'HEAD'])
Log.progress(`presearch-core repo at ${braveCoreDir} is at commit ID ${braveCoreSha}`)
